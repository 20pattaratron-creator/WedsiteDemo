...
const result={version:'3.6.0',packageVersion:'1.6.0',checkedAt:new Date().toISOString(),runtimeRootFiles:runtimeRoot.length,jsFiles:jsFiles.length,inlineHandlerFunctions:inlineCalls.size,issues,notes,status:issues.some(x=>x.severity==='error')?'FAIL':'PASS'};
fs.writeFileSync(path.join(ROOT,'CODEBASE_AUDIT_RESULTS.json'),JSON.stringify(result,null,2));
console.log(`ERP Codebase Audit 3.6.0: ${result.status}`);
