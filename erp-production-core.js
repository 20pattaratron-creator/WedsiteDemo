...
function validateInvoiceStock(branch,items,editId='',sourceOrderId=null){
  const totals=new Map(),restored=new Map();let orderId=sourceOrderId===null?(window.ERPWorkflowContext?.preparedSalesOrderId||''):sourceOrderId;
...
function installNavigationHook(){if(window.__ERP_PRODCORE_NAV_HOOK__)return false;window.__ERP_PRODCORE_NAV_HOOK__=true;document.addEventListener('erp:navigation',event=>renderPage(event?.detail?.id||''));return true;}
...
if(!window.__ERP_PRODCORE_ACTIONS_BOUND__){
  window.__ERP_PRODCORE_ACTIONS_BOUND__=true;
  document.addEventListener('click',event=>{
    const btn=event.target.closest?.('[data-prodcore-action]');if(!btn)return;
    const id=btn.dataset.recordId||'';
    const actions={'use-po':()=>window.pcUsePoForReceipt?.(id),'cancel-po':()=>window.pcCancelPo?.(id),'reverse-gr':()=>window.pcReverseGr?.(id),'restore-trash':()=>window.pcRestoreTrash?.(id)};
    actions[btn.dataset.prodcoreAction]?.();
  });
}
