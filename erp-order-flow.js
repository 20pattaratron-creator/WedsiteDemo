...
(function () {
  ...
  function ensureNavAndPanel() {
    ...
    document.dispatchEvent(new Event('erp:flow-ready'));
    return true;
  }
  ...
  function createProduction(orderId) {
    ...
    if (typeof window.useQuoteForProduction === 'function' && order.sourceQuoteId) {
      ...
      (window.ERPWorkflowContext ||= {preparedSalesOrderId:'',preparedProductionOrderId:''}).preparedProductionOrderId=order.id;
  ...
  function prepareDelivery(orderId) {
    ...
    (window.ERPWorkflowContext ||= {preparedSalesOrderId:'',preparedProductionOrderId:''}).preparedSalesOrderId=order.id;
  ...
  function init() {
    ...
    document.addEventListener('erp:navigation', event => {
      if (event?.detail?.id !== 'order-flow') document.getElementById('erp-flow-nav')?.classList.remove('active');
    });
    renderDashboardQueue();
  }
